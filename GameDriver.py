...
class GameDriver:
    ...
    def run(self):
        ...
        while True:
            if self.board.has_legal_moves_remaining(current.symbol):    # checks if any legal moves can be made
                cant_move_counter = 0
                self.process_move(current, opponent)
                self.board.display()
            else:   # If each player can't move then the game is over
                print("Can't move")
...
def main():
    ...
    #game = GameDriver("minimax", "minimax", 4, 4)
    game.run()
    return 0
